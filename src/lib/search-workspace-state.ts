...
function normalizeParams(raw: unknown): SearchWorkspaceStoredParams {
  ...
  const q = typeof value.q === "string" && value.q.trim().length > 0 ? value.q : undefined
  const tags =
    typeof value.tags === "string" && value.tags.trim().length > 0 ? value.tags : undefined
  const mode = value.mode === "all" ? "all" : value.mode === "any" ? "any" : undefined
  const noteId =
    typeof value.noteId === "string" && value.noteId.trim().length > 0 ? value.noteId : undefined
...
export function writeSearchWorkspaceParams(params: SearchWorkspaceStoredParams) {
  if (!hasWindow()) return
  ...
  try {
    window.localStorage.setItem(SEARCH_WORKSPACE_STORAGE_KEY, JSON.stringify(next))
  } catch {
